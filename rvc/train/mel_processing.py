...
class MultiScaleMelSpectrogramLoss(torch.nn.Module):
    ...
    def gen_images(self, real, fake):
        n_mels = [40, 160, 480]
        images = {}
        with torch.no_grad():
            for n in n_mels:
                real_mels = self.mel_spectrogram(real, **self.stft_params[n])
                fake_mels = self.mel_spectrogram(fake, **self.stft_params[n])
                real_logmels = torch.log(real_mels.clamp(min=1e-5).pow(1)) / self.log_base
                fake_logmels = torch.log(fake_mels.clamp(min=1e-5).pow(1)) / self.log_base
                images[n] = {"r":real_logmels, "g":fake_logmels}
        return n_mels, images